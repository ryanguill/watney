	...
	return function init( _bot ) {
		...
		bot.wrapChannel = function wrapChannel (channel) {
			...
			channel.send = function(text, dontLog) {
				...
				if (!dontLog) {
					...
					bot.logMessage(channel.name, bot.botName, text);
				}
		...
		function pauseChannel (channel, user) {
			bot.ops.isOp(user.name, (err, data) => {
				...
				return channel.send('I have been blocked from responding to all input in ' + channel.name + '. An OP must issue !play for me to resume.');
			});
		}
