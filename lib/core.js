...
module.exports = (function(){
	...
	return function init( _bot ) {
		...
		var matchPattern = function matchPattern (pattern, message, channel, user) {
			...
			//if you run the same exact regex command input twice in a row, it wont match the second time - I don't
			//know why - seems like some sort of weird cache?
			...
			if (_.has(pattern, 'regex')) {
				if (pattern.regex.test(message.text) !== true) {
					//console.log('regex', pattern.regex, pattern.regex.test(message.text));
					return false;
				}
			}
		...
		bot.on('presenceChange', function(user, presence) {
			console.log('presenceChange', user.name, presence);

			var patterns = getMatchingPatterns('presenceChange', presence, null, user);

			bot.log('presenceChange/patternCount', presence, patterns.length, patterns);
